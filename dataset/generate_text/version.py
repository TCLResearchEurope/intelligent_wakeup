...
Version management for text generation pipeline.
"""

# Semantic versioning for text generation code
# Format: MAJOR.MINOR.PATCH
#
# MAJOR: Breaking changes to output format, agent behavior changes
# MINOR: New features, scenario additions, non-breaking improvements (triggers regeneration)
# PATCH: Bug fixes, prompt tweaks, minor adjustments (no regeneration required)
VERSION = "0.1.0"


def get_version() -> str:
    """
    Get the current version of the text generation pipeline.
...
def should_regenerate(old_version: str, new_version: str) -> bool:
    ...
    try:
        ...
        # Regenerate if major or minor version changed
        return old_major != new_major or old_minor != new_minor
    except (ValueError, AttributeError):
        # If we can't parse version, assume regeneration is needed
        return True
